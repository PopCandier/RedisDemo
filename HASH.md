...
最开始，redis的hash默认使用的是ht[0],ht[1]不会初始化和分空间。

哈希表的dictht是用链地址法来解决碰撞问题，什么叫做链地址法来解决碰撞问题？就是dictht里面有一个dictEntry数组，数组里面有若干个节点，当有数据进行hash运算的时候，会计算出具体要落到哪个节点上，我们可以把这种情况称为挂载节点，不过有的时候可能两个数据进行运算后，落的节点是同一个，这就是发生了hash碰撞，所以这种情况下，就做成了链表的形式。这种算法的性能，和挂载的数量是有关系的，挂载的越多，性能也会变差

在这种情况下，哈希表的性能取决于它的大小(size)，和他所保存的节点数量(used)，之间的比率。

* 比率为 1：1时 （一个哈希表ht上一个节点上只有一个entry）
* 如果节点数量比哈希表大的多的时候，（这个比例用 ratio 表示，5 表示平均一个 ht 存储 5 个 entry），那么哈希表就会退化成多个链表，哈希表本身的性能优势就不再存在。）

在这种情况下需要扩容。Redis 里面的这种操作叫做 rehash。

* 为字符 ht[1]哈希表分配空间，这个哈希表的空间大小取决于要执行的操作，以
  及 ht[0]当前包含的键值对的数量。扩展：ht[1]的大小为第一个大于等于 ht[0].used*2。
* 将所有的 ht[0]上的节点 rehash 到 ht[1]上，重新计算 hash 值和索引，然后放
  入指定的位置。
* 当 ht[0]全部迁移到了 ht[1]之后，释放 ht[0]的空间，将 ht[1]设置为 ht[0]表，
...
dict_can_resize 为 1 并且 dict_force_resize_ratio 已使用节点数和字典大小之间的
比率超过 1：5，触发扩容